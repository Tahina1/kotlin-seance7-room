...
interface ProduitDao {
    ...
    // -----------------------------------------------------------------------
    //    Parce qu'en SQLite, prixKg IS NULL n'est pas une condition de filtrage ici
    //    mais une expression qui vaut 0 ou 1 : 0 quand le prix existe, 1 quand il est NULL.
    //    Comme ORDER BY trie en ascendant par défaut, les lignes à 0 passent avant celles à 1.
    //    Le second critère, prixKg DESC, ne s'applique qu'à l'intérieur de chaque groupe,
    //    et ordonne les prix connus du plus grand au plus petit.
    //    D'où : tous les prix décroissants, puis les NULL en bas.
    //    L'astuce est de fabriquer un premier niveau de tri à partir d'un booléen.

    // -----------------------------------------------------------------------
    // TODO 2 — FILTRE : les produits dont le stock dépasse un seuil donné,
    @Query("SELECT * FROM produits WHERE stockKg > :seuilKg ORDER BY stockKg DESC")
